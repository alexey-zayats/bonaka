...
HeatExchanger::HeatExchanger(QWidget *parent) : QWidget(parent)
{
    ...
    selectorView->setWindowTitle(tr("Расчет теплообменников"));
    selectorView->setAutoFillBackground(true);

    auto activeLabel = new QLabel(tr("Расчет теплообменников"));
